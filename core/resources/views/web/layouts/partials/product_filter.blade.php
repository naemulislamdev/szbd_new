<div class="filter-box category-filter-box" id="filter-box">
    <a data-bs-toggle="offcanvas" href="#productFilter" role="button" aria-controls="productFilter" class="btn"><i
                                        ...
                                        <div class="sub-category">
                                            <div class="sub-category-header">
                                                ...
                                                <span class="toggle-icon"
                                                    data-toggle="subCategory_{{ $subCat['name'] }}">+</span>
                                            </div>
                                            <div class="child-categories" id="subCategory_{{ $subCat['name'] }}">
                                                @if ($subCat->childes->count() > 0)
                                                    @foreach ($subCat->childes as $childCat)
                                                        <a
                                                            href="{{ route('category.products', [$category->slug, $subCat->slug, $childCat->slug]) }}">{{ $childCat['name'] }}</a><br>
                                                    @endforeach
                                                @endif
                                            </div>
                                        </div>
