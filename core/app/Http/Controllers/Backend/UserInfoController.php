...
elseif (isset($productDetails[0]) && is_array($productDetails[0])) {
    ...
    foreach ($productDetails as $item) {
        ...
        $html .= '<div class="mb-1" style="border-bottom:1px solid #eee;padding-bottom:5px;">';
        $html .= '<strong>Product Code:</strong> ' . ($product->code ?? 'N/A') . '<br>';
        $html .= '<strong>Product Price:</strong> ' . ($product->unit_price ?? 'N/A') . '<br>';
        $html .= '<strong>Qty:</strong> ' . $qty . '<br>';

        if (!empty($size)) {
            foreach ($size as $choiceName => $choiceVal) {
                $html .= '<strong>' . $choiceName . ':</strong> ' . $choiceVal . '<br>';
            }
        }
