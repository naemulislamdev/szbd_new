   ...
   public function set_shipping_method(Request $request)
    ...
    public function productCheckout(Request $request)
    {
        ...
        try {
            ...
            $promoProducts = ['HG1999D', 'HG1999E', 'HG1999G', 'HG1999BL', 'HG1999BK', 'HG1999S', 'HG1999BN', 'HG1999R', 'HG1999BY', 'G1399LB','G1399BY','G1399PP'];
            $promoQty = 0;
            $promoItems = [];
...
$lpSubTotal         = 0;
$nonDiscountedTotal = 0; // শুধু discount-ছাড়া product গুলোর total
...
foreach ($selectedItems as $productId => $quantity) {
    ...
    $discountAmt = 0;
    if ($p->discount > 0) {
        $discountAmt = $p->discount_type === 'flat'
            ? $p->discount
            : ($p->unit_price / 100) * $p->discount;
    }
    $finalPrice = $p->unit_price - $discountAmt;

    $lpSubTotal += $finalPrice * $qty;

    // শুধু discount নেই এমন product গুলো nonDiscountedTotal-এ যোগ হবে
    if ($discountAmt <= 0) {
        $nonDiscountedTotal += $finalPrice * $qty;
    }
}
...
if ($config->shipping_type === 'free_shipping') {
    if ($config->free_shipping_type === 'all_products') {
        ...
        $freeMin = (float) ($config->free_shipping_min_amount ?? 0);
        // *** KEY CHANGE: $lpSubTotal এর বদলে $nonDiscountedTotal দিয়ে check ***
        $baseCost = ($freeMin > 0 && $nonDiscountedTotal >= $freeMin) ? 0 : $shippingMethod->cost;
    }
}
