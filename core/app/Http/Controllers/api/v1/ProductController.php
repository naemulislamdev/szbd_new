...
class ProductController extends Controller
{
    ...
    {
        $allProducts = Product::with(['reviews'])->active()->get();
        return response()->json($allProducts, 200);
    }

    public function get_trending_keywords()
    {
        $keywords = TrendingKeyword::where('is_active', 1)->orderBy('sort_order')->pluck('keyword');
    ...
    public function submit_product_review(Request $request)
    {
        ...
        if (!empty($request->file('fileUpload'))) {
            foreach ($request->file('fileUpload') as $image) {
                if ($image != null) {
                    array_push($image_array, FileManager::uploadFile('review/', 300, $image));
                }
            }
        }
