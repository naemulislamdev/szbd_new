...
class PassportAuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (isset($user) && $user->is_active && auth()->guard('customer')->attempt($data)) {
            ...
            $user1=auth()->guard('customer')->user();
    ...
    // send otp to mobile no using sms apitemporary_token
        ...
        if (isset($customerCheck)) {
            ...
            return response()->json(['message' => 'Send OTP your phone Number !','success'=>'1'
], 200);
        } else {
            $errors = [];
            array_push($errors, ['code' => 'auth-001', 'message' => 'Customer_not_found_or_Account_has_been_suspended']);
        ...
        // Default password = phone number, so the account is usable via
        // phone+password login while OTP/SMS is off (user can change it later).
    ...
    public function recivedOTP(Request $request){
        if($request->otp){
        // Bind the OTP to the phone when the client supplies it (secure path).
        // Older app builds send otp only; that path still works for compatibility.
        $finduser = User::where('otp',$request->otp);
        if($request->phone){
            $finduser = $finduser->where('phone',$request->phone);
        }
        $finduser = $finduser->first();

        // No user matches this OTP (and phone, if given) -> reject.
        if(!$finduser){
            $errors = [];
            array_push($errors, ['code' => 'auth-001', 'message' => 'Invalid or expired OTP']);
            return response()->json([
                'errors' => $errors
            ], 401);
        }

        // Single-use: clear the OTP so it cannot be replayed or brute-forced
        // after a successful login.
        $finduser->otp = null;
        $finduser->temporary_token = null;
        $finduser->save();
