    ...
    public static function set_data_format($data)
{
    try {
        ...
        // images
        $data['images'] = is_string($data['images'])
            ? json_decode($data['images'], true)
            : $data['images'];

        // colors
        $colors = is_string($data['colors'])
            ? json_decode($data['colors'], true)
            : $data['colors'];

        $data['colors'] = !empty($colors)
            ? Color::whereIn('code', $colors)->get(['name', 'code'])
            : [];

        // attributes
        $attributesRaw = is_string($data['attributes'])
        ...
        // choice options
        $data['choice_options'] = is_string($data['choice_options'])
            ? json_decode($data['choice_options'], true)
            : $data['choice_options'];

        // variation
        $variationRaw = is_string($data['variation'])
            ? json_decode($data['variation'], true)
            : $data['variation'];

        if (!empty($variationRaw)) {
            foreach ($variationRaw as $var) {
                $variation[] = [
                    'type' => $var['type'] ?? null,
