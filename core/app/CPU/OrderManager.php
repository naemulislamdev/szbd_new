...
class OrderManager
{
    ...
    public static function generate_order($data)
    {
        ...
        $inserted_order_id = DB::table('orders')->insertGetId($or);

        foreach ($req['cart'] as $c) {
            $product = Product::where(['id' => $c['product_id']])->first();

            // Derive the ordered colour's image so the admin order page shows
            // the picked colour (the app sends the colour name in `variant`,
            // never the colour image). Mirrors the web order flow.
            $color_image = null;
            $color_variant = json_decode($product->color_variant ?? '[]', true);
            if (is_array($color_variant) && !empty($c['variant'])) {
                foreach ($color_variant as $cv_row) {
                    if (($cv_row['color'] ?? null) == $c['variant']
                        || ($cv_row['code'] ?? null) == $c['variant']) {
                        $color_image = $cv_row['image'] ?? null;
                        break;
                    }
                }
            }
