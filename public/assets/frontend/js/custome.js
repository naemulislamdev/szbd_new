new Swiper(".leftPromo", {
  ...
  pagination: {
    el: ".leftPromo .swiper-pagination",
    clickable: true,
  },
});

new Swiper(".rightPromo", {
  loop: true,
  speed: 800,
  autoplay: {
    delay: 4000,
...
// Fetatured and Categories product slider script
new Swiper(".ProductSlider", {
  loop: true,
    loopedSlides: 5,  // 👈 এটা যোগ করুন
...
document.querySelectorAll(".CategoryProductSlider").forEach((slider) => {
  ...
  new Swiper(slider, {
    loop: true,
  loopedSlides: slideCount > 5,  // 👈 এটা যোগ করুন
...
function scrollText() {
  document.title =
    scrollTitle.substring(position) + " " + scrollTitle.substring(0, position);
  position++;
...
$(document).ready(function () {
  $(".grid-btn-mobile").on("click", function () {
    ...
    $('.product-column[data-category="' + category + '"]')
      .removeClass(
        "col-md-2 col-md-3 col-md-4 col-md-5 col-md-6 col-sm-12 col-sm-6",
      )
      .addClass("col-sm-" + columns);

    $('.grid-btn-mobile[data-category="' + category + '"]').removeClass(
      "active",
    );
